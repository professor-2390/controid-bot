  ...
  data: new SlashCommandBuilder()
    ...
    .addNumberOption((option) =>
      option
        .setName("minutes")
        .setDescription(
          `The amount of minutes that you want the user to stay muted.`
        )
        .setRequired(true)
    )
  ...
  async execute(interaction) {
    ...
    if (mins > 720 || mins <= 0) {
      return interaction.reply({
        content: `Minutes must be a positive number lower than 720.`,
        ephemeral: true,
      });
    }
    ...
    if (member.id == interaction.member.user.id) {
      return interaction.reply({
        content: `You can't mute youself, smarty pants!`,
        ephemeral: true,
      });
    }
    ...
    if (
      ...
      return interaction.reply({
        content: `Your roles must be higher than the roles of the person you want to mute!`,
        ephemeral: true,
      });
    }
    ...
    const muteEmbed = new MessageEmbed()
      ...
      .setFooter(
        `You can use /unmute to unmute the user earlier than ${mins} minutes and /muteinfo to view information about his mute.`
      )
      .setTimestamp();
