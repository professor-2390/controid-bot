...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("user")
        .setDescription(
          "The user that you want to view information about his ban."
        )
        .setRequired(true)
    ),
  ...
  async execute(interaction) {
    ...
    let bannedUser;
    if (bannedUsersArr)
      bannedUser = bannedUsersArr.find((user) => user.username === username);
    if (!bannedUser) {
    ...
    if (bannedUser.unbanDate) {
      ...
      const daysRemaining = Math.ceil(
        (bannedUser.unbanDate - Date.now()) / millisecondsPerDay
      );
