    ...
    if (command.requiredPerms) {
      command.requiredPerms.forEach(async (perm) => {
        if (!interaction.member.permissions.has(perm)) {
          err = 1;
          interaction.reply({
            content: `You need the following permission to run this: ${
              "`" + perm + "`"
            }`,
            ephemeral: true,
          });
    ...
    if (command.botRequiredPerms) {
      command.botRequiredPerms.forEach(async (perm) => {
        if (!interaction.guild.me.permissions.has(perm)) {
          err = 1;
          interaction.reply({
            content: `I need the following permission to run this: ${
              "`" + perm + "`"
            }`,
            ephemeral: true,
          });
    ...
    if (interaction.customId === "role_picker") {
      ...
      interaction.values.forEach((roleId) => {
        ...
        if (interaction.guild.me.roles.highest.comparePositionTo(role) <= 0) {
          error = 1;
          return interaction.reply({
            content: `Error at role ${
              "`" + role.name + "`"
            }. My roles must be higher than the role that you want to give yourself.`,
            ephemeral: true,
          });
        }
      ...
      interaction.component.options
        ...
        .forEach((roleId) => {
          if (
            ...
            !interaction.values.includes(roleId)
          ) {
            const role = interaction.guild.roles.cache.get(roleId);
            if (
              interaction.guild.me.roles.highest.comparePositionTo(role) <= 0
            ) {
              error = 1;
              return interaction.reply({
                content: `Error at role ${
                  "`" + role.name + "`"
                }. My roles must be higher than the role that you want to remove from yourself.`,
                ephemeral: true,
              });
            }
      ...
      if (!error) {
        interaction.reply({
          content: `Successfully updated your roles.`,
          ephemeral: true,
        });
      }
    }
